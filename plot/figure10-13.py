...
for cycle in range(22): #0-22
	...
	#x[cycle] = " "
	SPA[cycle] = (float)(content[cycle][7])
	HASH[cycle] = (float)(content[cycle][11])
	ESC[cycle] = (float)(content[cycle][15])
	oneMKL[cycle] = (float)(content[cycle][19])
	HASpGEMM[cycle] = (float)(content[cycle][3])
...
fig.savefig("figure-10-13.eps",dpi=600,format='eps',bbox_inches = 'tight')
plt.show()
plt.close()
